...
from db import init_db, get_settings, upsert_settings, get_open_ticket, add_open_ticket, remove_open_ticket
...
COLOR_SUCCESS = 0x121212   # 완료/성공 안내 다크
...
async def 설정(interaction: discord.Interaction):
    row = await get_settings(bot.db, interaction.guild_id)
    if not row:
        await interaction.response.send_message(
            embed=eb('설정', '아직 설정이 없네. /설정_메뉴로 먼저 설정해줘.', COLOR_ACCENT),
            ephemeral=True
        )
        return
    manager_role_id, category_id, log_channel_id = row
    desc = (
        f'티켓 관리자 역할: {f"<@&{manager_role_id}>" if manager_role_id else "없음"}\n'
        f'티켓 카테고리: {f"<#{category_id}>" if category_id else "없음"}\n'
        f'로그 채널: {f"<#{log_channel_id}>" if log_channel_id else "없음"}'
    )
    await interaction.response.send_message(embed=eb('설정', desc, COLOR_ACCENT), ephemeral=True)
...
async def 티켓_임베드_생성(interaction: discord.Interaction, 채널: discord.TextChannel):
    ...
    await 채널.send(embed=eb('티켓 생성', '아래 버튼을 눌러 티켓을 만들어줘.', COLOR_ACCENT), view=view)
    await interaction.response.send_message(
        embed=eb('성공', f'티켓 임베드가 {채널.mention}에 생성됐어.', COLOR_SUCCESS),
        ephemeral=True
    )

# ── 설정 메뉴 View(모든 응답 임베드)
class SettingMenu(discord.ui.View):
    ...
    async def set_role(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def on_select(i: discord.Interaction):
            ...
            row = await get_settings(bot.db, i.guild_id) or (None, None, None)
            _, category_id, log_channel_id = row
            await upsert_settings(bot.db, i.guild_id, role_id, category_id, log_channel_id)
    ...
    async def set_category(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def on_select(i: discord.Interaction):
            ...
            await upsert_settings(bot.db, i.guild_id, manager_role_id, category_id, log_channel_id)
    ...
    async def set_log(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def on_select(i: discord.Interaction):
            ...
            await upsert_settings(bot.db, i.guild_id, manager_role_id, category_id, log_id)
...
class TicketCreateView(discord.ui.View):
    ...
    async def create(self, interaction: discord.Interaction, button: discord.ui.Button):
        row = await get_settings(bot.db, interaction.guild_id)
        if not row:
            await interaction.response.send_message(embed=eb('미설정', '아직 설정이 안 되어 있어. /설정_메뉴 먼저.'), ephemeral=True)
            return
        manager_role_id, category_id, log_channel_id = row

        existing = await get_open_ticket(bot.db, interaction.guild_id, interaction.user.id)
        if existing:
            ch = interaction.guild.get_channel(int(existing))
            if ch:
                await interaction.response.send_message(embed=eb('중복 방지', f'이미 열린 티켓이 있어: {ch.mention}', COLOR_WARN), ephemeral=True)
                return
            else:
                await remove_open_ticket(bot.db, interaction.guild_id, interaction.user.id)
        ...
        # 채널 이름을 ID 기반으로 바꾸고 싶으면 아래 한 줄 교체:
        ...
        await add_open_ticket(bot.db, interaction.guild_id, interaction.user.id, channel.id)
        ...
        await channel.send(content=(' '.join(x for x in [interaction.user.mention, manager_role_mention] if x)).strip(), embed=first, view=close_view)

        jump = f'https://discord.com/channels/{interaction.guild_id}/{channel.id}'
        await interaction.response.send_message(embed=eb('티켓 생성 완료', f'{channel.mention}에서 이어서 대화해줘.' , COLOR_SUCCESS), ephemeral=True, view=discord.ui.View().add_item(discord.ui.Button(label='티켓 바로가기', url=jump, style=discord.ButtonStyle.link)))
...
class CloseConfirmView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await remove_open_ticket(bot.db, interaction.guild_id, self.owner_id)
